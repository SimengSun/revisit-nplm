...
class AdaptiveSoftmax(nn.Module):
	def __init__(self, n_token, d_embed, d_proj, cutoffs, div_val=1,
		...
		self.out_projs = nn.ModuleList() # changing to moduleList because current pytorch does not replicate parameterlist when using multi-gpu

		if div_val == 1:
			for i in range(len(self.cutoffs)):
				if d_proj != d_embed:
					self.out_projs.append(
						nn.Linear(d_proj, d_embed)
					)
				else:
	...
	def forward(self, hidden, target, keep_order=False, return_rank=False):
		...
		if self.n_clusters == 0:
			...
			if return_rank: # another pass to get the rank
